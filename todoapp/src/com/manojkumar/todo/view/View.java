...
import com.manojkumar.todo.customid.CustomId;
import com.manojkumar.todo.customid.GetCustomId;
...
public class View {
	...
	/*
	 * When all the task are deleted then the id should start with 1 again. So this
	 * method will do this.
	 */
	private static void resetId() throws IOException, ClassNotFoundException {
		GetCustomId obj = GetCustomId.getInstance();
		ArrayList<ToDoDTO> tasks = new ArrayList<>();
		ITodoRepo repo = ToDoRepo.getInstance();
		tasks = repo.printAllTask();
		if (tasks.size() == 0) {
			obj.addId(new CustomId(0));
		}
	}
	...
	private static void addTask() {
		...
		System.out.println(getValue("task.desc"));
		String desc = scanner.nextLine();
		int systemGenerateId = 0;
		ToDoDTO todo = null;
		/*
		 * Id work
		 */

		CustomId idObj = new CustomId(1);
		GetCustomId customIdObj;
		try {

			customIdObj = GetCustomId.getInstance();
			CustomId oldIdObj = customIdObj.returnIdObj();
			systemGenerateId = idObj.getId() + oldIdObj.getId();
			customIdObj.addId(new CustomId(systemGenerateId));
		} catch (EOFException e) {
			try {
				customIdObj = GetCustomId.getInstance();
				customIdObj.addId(idObj);
